...
final class Property_Frontend {
	...
	public static function enqueue_assets(): void {
		if ( is_singular( 'skt_property' ) || is_post_type_archive( 'skt_property' ) || is_tax( array( 'skt_property_city', 'skt_property_type' ) ) ) {
			wp_enqueue_style( 'smartkey-property-templates', plugins_url( 'assets/css/property-templates.css', SKT_CORE_FILE ), array(), SKT_CORE_VERSION );
		}
	}
	...
	public static function render_archive(): string {
		...
		$intro   = $is_city
			? sprintf( 'Explore property opportunities in %s through a structured advisory journey. Availability, ownership, title restrictions, pricing and transaction eligibility are verified for each inquiry.', $city->name )
			: 'Compare property opportunities across Istanbul, Ankara, Izmir and Antalya. SmartKeyTurkey coordinates discovery and due diligence as an intermediary; it does not own the listed properties.';
	...
	private static function render_card(): void {
		...
		$price   = get_post_meta( $post_id, 'skt_property_price', true );
		$currency = get_post_meta( $post_id, 'skt_property_currency', true );
		?>
		<article <?php post_class( 'skt-property-card' ); ?>><a class="skt-property-card-image" href="<?php the_permalink(); ?>"><?php if ( has_post_thumbnail() ) : the_post_thumbnail( 'medium_large', array( 'loading' => 'lazy' ) ); else : ?><span aria-hidden="true">SK</span><?php endif; ?></a><div><p class="skt-property-card-city"><?php echo esc_html( $city && ! is_wp_error( $city ) ? $city[0]->name : 'Turkey' ); ?></p><h3><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3><p><?php echo esc_html( wp_trim_words( get_the_excerpt(), 22 ) ); ?></p><strong><?php echo esc_html( $price ? trim( $currency . ' ' . $price ) : 'Price confirmed on inquiry' ); ?></strong></div></article>
		<?php
	}
	...
	public static function render_single(): string {
		...
		$fields = array(
			'Rooms' => 'skt_property_rooms', 'Bathrooms' => 'skt_property_bathrooms', 'Gross area' => 'skt_property_gross_area', 'Net area' => 'skt_property_net_area',
			'Completion' => 'skt_property_completion_status', 'Title status' => 'skt_property_title_status', 'Availability' => 'skt_property_availability', 'Citizenship review' => 'skt_property_citizenship_review',
		);
		$cities = get_the_terms( $id, 'skt_property_city' );
		$city = $cities && ! is_wp_error( $cities ) ? $cities[0] : null;
		$disclosure = get_post_meta( $id, 'skt_property_representative_disclosure', true ) ?: 'SmartKeyTurkey acts as an intermediary and advisor and is not the property owner. Ownership, encumbrances, restrictions, availability, pricing and transaction eligibility require case-specific verification.';
		ob_start();
		?>
		<main class="skt-property-single" id="main-content"><section class="skt-property-hero"><div class="skt-property-shell"><nav class="skt-property-breadcrumbs" aria-label="Breadcrumb"><a href="<?php echo esc_url( home_url( '/' ) ); ?>">Home</a><span>/</span><a href="<?php echo esc_url( get_post_type_archive_link( 'skt_property' ) ); ?>">Properties</a><?php if ( $city ) : ?><span>/</span><a href="<?php echo esc_url( get_term_link( $city ) ); ?>"><?php echo esc_html( $city->name ); ?></a><?php endif; ?></nav><div class="skt-property-single-grid"><div class="skt-property-featured"><?php echo get_the_post_thumbnail( $id, 'large' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div><div><p class="skt-property-eyebrow"><?php echo esc_html( $city ? $city->name : 'Turkey property' ); ?></p><h1><?php echo esc_html( get_the_title( $id ) ); ?></h1><p><?php echo esc_html( get_the_excerpt( $id ) ); ?></p><a class="skt-property-button" href="#property-consultation">Request current details</a></div></div></div></section><section class="skt-property-shell skt-property-detail"><article><?php echo apply_filters( 'the_content', get_post_field( 'post_content', $id ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></article><aside><h2>Property facts</h2><dl><?php foreach ( $fields as $label => $key ) : $value = get_post_meta( $id, $key, true ); if ( $value ) : ?><div><dt><?php echo esc_html( $label ); ?></dt><dd><?php echo esc_html( $value ); ?></dd></div><?php endif; endforeach; ?></dl><p class="skt-property-disclosure"><?php echo esc_html( $disclosure ); ?></p></aside></section><?php echo self::render_inquiry_form( $id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></main>
		<?php
		return (string) ob_get_clean();
	}
	...
	private static function render_inquiry_form( int $property_id = 0 ): string {
		...
		ob_start(); ?>
		<section class="skt-property-inquiry" id="property-consultation"><div class="skt-property-shell"><div class="skt-property-section-heading"><div><p class="skt-property-eyebrow">Private consultation</p><h2>Request a property shortlist</h2></div><p>Share your requirements. No purchase, residence or citizenship outcome is guaranteed.</p></div><?php if ( 'sent' === $status ) : ?><p class="skt-property-notice" role="status">Thank you. Your inquiry has been sent for review.</p><?php elseif ( 'error' === $status ) : ?><p class="skt-property-notice is-error" role="alert">The inquiry could not be sent. Please review the required fields and try again.</p><?php endif; ?><form class="skt-property-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><?php wp_nonce_field( 'skt_property_inquiry', 'skt_property_nonce' ); ?><input type="hidden" name="action" value="skt_property_inquiry"><input type="hidden" name="property_id" value="<?php echo esc_attr( $property_id ); ?>"><label>Name *<input name="name" required autocomplete="name"></label><label>Email *<input type="email" name="email" required autocomplete="email"></label><label>Phone / WhatsApp<input name="phone" autocomplete="tel"></label><label>Preferred city<select name="city"><option value="">Select a city</option><?php foreach ( array( 'Istanbul', 'Ankara', 'Izmir', 'Antalya' ) as $city ) : ?><option><?php echo esc_html( $city ); ?></option><?php endforeach; ?></select></label><label>Approximate budget<input name="budget" placeholder="Currency and range"></label><label>Intended use<select name="purpose"><option value="">Select intended use</option><option>Primary residence</option><option>Investment</option><option>Holiday home</option><option>Other</option></select></label><label class="skt-property-form-wide">Requirements<textarea name="requirements" rows="5" placeholder="Location, property type, rooms, timing and any due-diligence questions"></textarea></label><label class="skt-property-consent skt-property-form-wide"><input type="checkbox" name="consent" value="1" required> I agree that SmartKeyTurkey may use these details to respond to this inquiry. *</label><label class="skt-property-hp" aria-hidden="true">Website<input name="website" tabindex="-1" autocomplete="off"></label><button class="skt-property-button" type="submit">Send consultation request</button></form></div></section>
	...
	public static function handle_inquiry(): void {
		...
		$data = array( 'Property' => $property_id ? get_the_title( $property_id ) : 'General shortlist', 'Name' => $name, 'Email' => $email, 'Phone' => sanitize_text_field( wp_unslash( $_POST['phone'] ?? '' ) ), 'City' => sanitize_text_field( wp_unslash( $_POST['city'] ?? '' ) ), 'Budget' => sanitize_text_field( wp_unslash( $_POST['budget'] ?? '' ) ), 'Purpose' => sanitize_text_field( wp_unslash( $_POST['purpose'] ?? '' ) ), 'Requirements' => sanitize_textarea_field( wp_unslash( $_POST['requirements'] ?? '' ) ) );
