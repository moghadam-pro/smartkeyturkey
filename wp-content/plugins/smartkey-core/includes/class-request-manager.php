...
final class Request_Manager {
	...
	public static function register_post_type(): void {
		register_post_type(
			'skt_request',
			array(
				'labels' => array( 'name' => 'Requests', 'singular_name' => 'Request', 'edit_item' => 'View Request', 'search_items' => 'Search Requests', 'not_found' => 'No requests found' ),
	...
	public static function capture_rfq( $contact_form, &$abort, $submission ): void {
		if ( ! $submission && class_exists( '\WPCF7_Submission' ) ) { $submission = \WPCF7_Submission::get_instance(); }
		if ( ! self::is_rfq_form( $contact_form ) || ! $submission ) { return; }
		$posted = $submission->get_posted_data();
		$data = array();
		foreach ( (array) $posted as $key => $value ) {
			if ( str_starts_with( (string) $key, '_' ) || 'consent' === $key ) { continue; }
	...
	public static function store_property( int $property_id, array $data ): int {
		$title = $property_id ? get_the_title( $property_id ) : 'General property request';
		return self::store( 'property', 'Property inquiry — ' . $title, $data, $property_id );
	}

	private static function store( string $type, string $title, array $data, int $related_id = 0 ): int {
		$request_id = wp_insert_post( array( 'post_type' => 'skt_request', 'post_status' => 'publish', 'post_title' => wp_strip_all_tags( $title ) ), true );
		if ( is_wp_error( $request_id ) ) { return 0; }
	...
	public static function columns( array $columns ): array {
		return array( 'cb' => $columns['cb'] ?? '', 'title' => 'Request', 'skt_type' => 'Type', 'skt_contact' => 'Contact', 'skt_status' => 'Status', 'date' => 'Received' );
	}

	public static function column_content( string $column, int $post_id ): void {
		$data = get_post_meta( $post_id, 'skt_request_data', true );
		$data = is_array( $data ) ? $data : array();
		if ( 'skt_type' === $column ) { echo esc_html( ucfirst( (string) get_post_meta( $post_id, 'skt_request_type', true ) ) ); }
		if ( 'skt_contact' === $column ) { echo esc_html( (string) ( $data['name'] ?? $data['full-name'] ?? $data['your-name'] ?? '' ) ); if ( ! empty( $data['email'] ) || ! empty( $data['business-email'] ) || ! empty( $data['your-email'] ) ) { echo '<br><small>' . esc_html( (string) ( $data['email'] ?? $data['business-email'] ?? $data['your-email'] ) ) . '</small>'; } }
		if ( 'skt_status' === $column ) { echo '<strong>' . esc_html( ucfirst( (string) get_post_meta( $post_id, 'skt_request_status', true ) ) ) . '</strong>'; }
	}

	public static function add_details_box(): void {
		add_meta_box( 'skt_request_details', 'Submitted details', array( self::class, 'render_details_box' ), 'skt_request', 'normal', 'high' );
	}
	...
	public static function render_details_box( \WP_Post $post ): void {
		...
		foreach ( $data as $key => $value ) { echo '<tr><th style="width:220px">' . esc_html( ucwords( str_replace( array( '-', '_' ), ' ', (string) $key ) ) ) . '</th><td style="white-space:pre-wrap">' . esc_html( is_array( $value ) ? implode( ', ', $value ) : (string) $value ) . '</td></tr>'; }
		echo '</tbody></table><p><small>Stored inside WordPress. No email is sent for SmartKeyTurkey website request forms.</small></p>';
	}
