...
final class Telegram_Bot {
	...
	private const VIDEO_MAX_SECONDS = 300;
	...
	private static function handle_update( array $update ): void {
		...
		if ( 'Show Draft Properties' === $text ) { self::show_drafts( $chat_id ); return; }
		if ( 'Find Property' === $text ) { self::start_find( $chat_id, $user_id ); return; }
	...
	private static function menu( int $chat_id ): void {
		self::send( $chat_id, 'SmartKeyTurkey Operations', array( 'keyboard' => array( array( array( 'text' => 'Add New Property' ) ), array( array( 'text' => 'Show Draft Properties' ), array( 'text' => 'Find Property' ) ) ), 'resize_keyboard' => true ) );
	}
	...
	private static function find_property( int $chat_id, int $user_id, string $text, int $message_id ): void {
		...
		if ( ! $post || 'skt_property' !== $post->post_type ) { self::send( $chat_id, 'Property not found.' ); self::menu( $chat_id ); return; }
	...
	private static function show_drafts( int $chat_id ): void {
		...
		foreach ( $posts as $post ) { self::send( $chat_id, '#' . $post->ID . ' — ' . $post->post_title, array( 'inline_keyboard' => array( array( array( 'text' => 'Set to Publish', 'callback_data' => 'status:' . $post->ID . ':publish' ) ) ) ) ); }
	...
	private static function questions(): array {
		return array(
			...
			'skt_property_latitude' => array( 'label' => 'Latitude', 'question' => 'Enter latitude.', 'type' => 'decimal', 'required' => false ),
	...
	private static function validate_answer( string $text, array $question ) {
		...
		if ( 'area' === $question['type'] ) { if ( ! is_numeric( str_replace( ',', '.', $text ) ) ) { return new \WP_Error( 'number', 'Enter the area as a number only. The bot adds m² automatically.' ); } return rtrim( rtrim( str_replace( ',', '.', $text ), '0' ), '.' ) . ' m²'; }
	...
	private static function render_form( int $chat_id, array $state ): void {
		...
		$markup = 'media' === $state['step'] ? array( 'inline_keyboard' => array( array( array( 'text' => 'Finish Media', 'callback_data' => 'finish_media' ) ), array( array( 'text' => 'Cancel', 'callback_data' => 'cancel_form' ) ) ) ) : self::question_markup( $state['step'] );
	...
	private static function form_text( array $data, string $step, int $media_count = 0 ): string {
		...
		$lines[] = ''; if ( 'media' === $step ) { $lines[] = 'Photos / videos received: ' . $media_count; $lines[] = 'Send photos or videos now. Videos: maximum 5 minutes and 20 MB (Telegram Cloud API limit).'; } else { $lines[] = 'NEXT'; $lines[] = self::questions()[ $step ]['question']; }
	...
	private static function question_markup( string $step ): array {
		...
		$rows = array_chunk( $buttons, 2 ); if ( empty( $q['required'] ) && 'yes_no_skip' !== $q['type'] ) { $rows[] = array( array( 'text' => 'SKIP', 'callback_data' => 'answer:' . $step . ':Skip' ) ); } $rows[] = array( array( 'text' => 'Cancel', 'callback_data' => 'cancel_form' ) );
	...
	private static function handle_media( int $chat_id, int $user_id, array $message, string $text, array $state ): void {
		...
		if ( ! empty( $message['video'] ) ) {
			...
			if ( $duration > self::VIDEO_MAX_SECONDS ) { self::send( $chat_id, 'Video rejected: duration must not exceed 5 minutes.' ); return; }
			if ( $size > self::VIDEO_MAX_BYTES ) { self::send( $chat_id, 'Video rejected: Telegram Cloud Bot API allows bots to download files only up to 20 MB. A local Bot API server is required for larger videos.' ); return; }
			$media = array( 'type' => 'video', 'file_id' => (string) $video['file_id'], 'size' => $size, 'duration' => $duration );
		}
		if ( ! $media ) { self::send( $chat_id, 'Send a photo or video, or tap Finish Media.' ); return; }
		$state['media'][] = $media; self::set_state( $user_id, $state ); self::delete_message( $chat_id, (int) ( $message['message_id'] ?? 0 ) ); self::render_form( $chat_id, $state );
	}
	...
	private static function create_property( int $chat_id, int $user_id, array $state ): void {
		...
		self::clear_state( $user_id ); self::send( $chat_id, sprintf( 'Property #%d was created as Draft with %d photo(s) and %d video(s).', $post_id, count( $photos ), count( $videos ) ) ); self::menu( $chat_id );
	}
	...
	private static function handle_callback( int $chat_id, int $user_id, string $data ): void {
		...
		if ( 'finish_media' === $data ) { $state = self::state( $user_id ); if ( empty( $state['media'] ) ) { self::send( $chat_id, 'Please send at least one photo or video.' ); return; } self::create_property( $chat_id, $user_id, $state ); return; }
